    ...
    <body>
        ...
        <form class="form-horizontal" action="<?php echo base_url(); ?>Ctrl_Details_Carburant/insert_Details_Carburant" method="post">
            <div class="form-group">
                <label class="col-sm-2 control-label">Nom du pompiste</label>
                <div class="col-sm-10">
                    <select name="nom_Pompiste" >
                    <?php for($i = 0;$i<count($data) ; $i++ ){  ?>
                        <option value="<?php echo $data[$i]->getNom_Pompiste();  ?>"><?php echo $data[$i]->getNom_Pompiste();  ?></option>
                    <?php }  ?>
                    </select>  
                </div>  
